@echo off
set "PORT=5007"
echo --- equilibr 🌪️ Next-Week-Proof Launcher (OFFLINE MODE) ---

:: Navigate to the main equilibr folder
pushd "%~dp0\.."
...
if '%errorlevel%' EQU '0' (
    ...
    :: Register Watchdog
    schtasks /create /tn "equilibr Watchdog" /tr "\"%~dp0..\health_check.bat\"" /sc DAILY /st 00:00 /F >nul 2>&1
...
echo ⏳ Syncing offline server certificate...
...
:: Launch Dashboard (Strictly Offline via Localhost)
...
echo.
echo ===================================
echo DONE! System is alive OFFLINE on: %URL%
echo ===================================
timeout /t 5
